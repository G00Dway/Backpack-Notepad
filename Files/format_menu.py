...
def main(root, text, menubar):
    ...
    formatMenu.add_command(label="Fon dəyişdirmək", command=objFormat.changeBg)
    formatMenu.add_command(label="Şrift rəngini dəyişdirmək", command=objFormat.changeFg)
    formatMenu.add_cascade(label="Şrift", underline=0, menu=fsubmenu)
    formatMenu.add_cascade(label="Ölçü", underline=0, menu=ssubmenu)
    formatMenu.add_command(label="Cılız", command=objFormat.bold, accelerator="Ctrl+B")
    formatMenu.add_command(label="Kursiv", command=objFormat.italic, accelerator="Ctrl+I")
    formatMenu.add_command(label="Vurğulamaq", command=objFormat.underline, accelerator="Ctrl+U")
    formatMenu.add_command(label="Üstündən sürtmək", command=objFormat.overstrike, accelerator="Ctrl+T")
    formatMenu.add_command(label="Tarix əlavə edin", command=objFormat.addDate)
